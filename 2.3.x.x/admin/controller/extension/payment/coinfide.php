...
class ControllerExtensionPaymentCoinfide extends Controller {

	private $error = array();

	public function index() {
		$this->load->language('extension/payment/coinfide');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/coinfide', 'token=' . $this->session->data['token'], true)
		);

		$this->load->model('localisation/order_status');
		$data['order_statuses'] = $this->model_localisation_order_status->getOrderStatuses();

		$data['action'] = $this->url->link('extension/payment/coinfide', 'token=' . $this->session->data['token'], true);
		...
		$this->response->setOutput($this->load->view('extension/payment/coinfide', $data));
	}
	...
	public function order() {
		...
		if ($this->config->get('coinfide_status')) {
			...
			$this->load->model('extension/payment/coinfide');

			$coinfide_order = $this->model_extension_payment_coinfide->getOrder($this->request->get['order_id']);

			if (!empty($coinfide_order)) {
				$this->load->language('extension/payment/coinfide');

				$coinfide_order['total_released'] = $this->model_extension_payment_coinfide->getTotalReleased($coinfide_order['coinfide_order_id']);
				...
				return $this->load->view('extension/payment/coinfide_order', $data);
			}
		}
	}
	...
	public function refund() {
		...
		if (isset($this->request->post['order_id']) && !empty($this->request->post['order_id'])) {
			...
			$coinfide_order = $this->model_extension_payment_coinfide->getOrder($this->request->post['order_id']);

//			$refund_response = $this->model_extension_payment_coinfide->refund($coinfide_order, $this->request->post['amount']);   //todo???

			$this->model_extension_payment_coinfide->logger($refund_response);

			if ($refund_response == 'ok') {
				$this->model_extension_payment_coinfide->addTransaction($coinfide_order['coinfide_order_id'], 'refund', $this->request->post['amount'] * -1);

				$total_refunded = $this->model_extension_payment_coinfide->getTotalRefunded($coinfide_order['coinfide_order_id']);
				$total_released = $this->model_extension_payment_coinfide->getTotalReleased($coinfide_order['coinfide_order_id']);

				if ($total_released <= 0 && $coinfide_order['release_status'] == 1) {
					$this->model_extension_payment_coinfide->updateRefundStatus($coinfide_order['coinfide_order_id'], 1);
					$refund_status = 1;
	...
	public function install() {
		$this->load->model('extension/payment/coinfide');
		$this->model_extension_payment_coinfide->install();
	}

	public function uninstall() {
		$this->load->model('extension/payment/coinfide');
		$this->model_extension_payment_coinfide->uninstall();
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/payment/coinfide')) {
