...
<div class="alert alert-success" id="coinfide_transaction_msg" style="display:none;"></div>
<table class="table table-striped table-bordered">
  <tr>
	<td><?php echo $text_order_ref; ?></td>
	<td><?php echo $coinfide_order['coinfide_transaction_id']; ?></td>
  </tr>
  <tr>
	<td><?php echo $text_order_total; ?></td>
	<td><?php echo $coinfide_order['total_formatted']; ?></td>
  </tr>
  <tr>
	<td><?php echo $text_total_released; ?></td>
	<td id="coinfide_total_released"><?php echo $coinfide_order['total_released_formatted']; ?></td>
  </tr>
  <tr>
	<td><?php echo $text_refund_status; ?></td>
	<td id="refund_status">
	  <?php if ($coinfide_order['refund_status'] == 1) { ?>
		  <span class="refund_text"><?php echo $text_yes; ?></span>
	  <?php } else { ?>
		  <span class="refund_text"><?php echo $text_no; ?></span>&nbsp;&nbsp;

		  <?php if ($coinfide_order['total_released'] > 0) { ?>
  ...
  <tr>
	...
	<td>
	  <table class="table table-striped table-bordered" id="coinfide_transactions">
		<thead>
		  <tr>
		...
		<tbody>
		  <?php foreach ($coinfide_order['transactions'] as $transaction) { ?>
			  <tr>
    ...
    $("#btn_refund").click(function () {
      if (confirm('<?php echo $text_confirm_refund ?>')) {
        $.ajax({
          ...
          url: 'index.php?route=extension/payment/coinfide/refund&token=<?php echo $token; ?>',
          beforeSend: function () {
            $('#btn_refund').hide();
            $('#refund_amount').hide();
            $('#img_loading_refund').show();
            $('#coinfide_transaction_msg').hide();
          },
          success: function (data) {
            if (data.error == false) {
              ...
              $('#coinfide_transactions').append(html);
              $('#coinfide_total_released').text(data.data.total_released);
              ...
              if (data.msg != '') {
                $('#coinfide_transaction_msg').empty().html('<i class="fa fa-check-circle"></i> ' + data.msg).fadeIn();
              }
            }
            if (data.error == true) {
              alert(data.msg);
