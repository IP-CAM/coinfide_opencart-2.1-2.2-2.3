...
class ControllerExtensionPaymentCoinfide extends Controller {
	public function index() {
		...
		$data['action'] = $this->url->link('extension/payment/coinfide/checkout', '', true);

		return $this->load->view('extension/payment/coinfide', $data);
	}
...
//                $buyer->setPhone($phone);
        ...
        $corder->setSuccessUrl($this->url->link('extension/payment/coinfide/success'));
...
//        echo $corder;
		...
		$this->model_extension_payment_coinfide->logger($corder);
		$this->model_extension_payment_coinfide->addCoinfideOrder($order_info);
...
//            echo 'Callback valid! You may process the order. Order data: ';
            $this->model_extension_payment_coinfide->logger('Callback valid! You may process the order. Order data: '.serialize($this->request->post));
			...
			if (isset($this->request->post['transactionUid'])) {
				...
				$coinfide_order_info = $this->model_extension_payment_coinfide->getCoinfideOrder($order_id);

				$this->model_extension_payment_coinfide->updateOrder($coinfide_order_info['coinfide_order_id'], $coinfide_transaction_id, 'payment', $order_info);
...
//                    $this->model_extension_payment_coinfide->logger('Hashes do not match, possible tampering!');
//                    return;
//                }
